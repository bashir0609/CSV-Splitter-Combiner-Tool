...
export async function POST(request: NextRequest) {
  try {
    ...
    lookupFile.data.forEach(row => {
      ...
      if (key) {
        if (vlookupConfig.matchType === 'exact') {
          ...
          // For approximate match, store with lowercase key for case-insensitive comparison
    ...
    // Generate filename
    let filename = customDownloadName || `vlookup-${vlookupConfig.lookupColumn}.csv`;
    ...
    console.error('Error performing VLOOKUP:', error);
    return NextResponse.json({ 
      error: 'Failed to perform VLOOKUP operation' 
    }, { status: 500 });
  }
}