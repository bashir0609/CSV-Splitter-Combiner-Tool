...
export function RemoveDuplicates() {
  ...
  }, [analysisData, mode, duplicateColumn, fileColumnMappings, handlePreview]);
  ...
  const canPreview = useMemo(() => {
    ...
    if (mode === 'single') {
      ...
      return analysisData.files.every(file => fileColumnMappings[file.filename]);
    }
  }, [analysisData, isLoading, mode, duplicateColumn, fileColumnMappings]);
  ...
  return (
    ...
    >
              ...
              <div className="space-y-4">
                  ...
                  {analysisData.files.map((file, index) => (
                    <div key={file.filename} className="bg-slate-800/50 rounded-lg p-4 border border-slate-600">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="font-medium text-slate-200">{file.filename}</h4>
                      ...
                      <div>
                        ...
                        {fileColumnMappings[file.filename] && (
                          <p className="text-xs text-green-400 mt-1">
                            ✓ Using "{fileColumnMappings[file.filename]}" for duplicate detection
                          </p>
                        )}
                      </div>
                    </div>
                ...
                <div className="bg-blue-900/20 border border-blue-700 rounded-lg p-3">
                  <p className="text-xs text-blue-300">
                    💡 <strong>Tip:</strong> Files will be merged first, then duplicates will be detected based on the selected columns. 
                    Choose columns that represent the same type of data across all files.
                  </p>
                </div>
              </div>
            )}
      ...
      {previewData && (
        <div className="space-y-6">
          <div className="bg-slate-700/50 rounded-lg p-4">
            <h3 className="text-lg font-medium text-slate-200 mb-3">Deduplication Results</h3>
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <span className="text-slate-300">Duplicates Found:</span>
                <span className="text-red-400 ml-2">{previewData.duplicateCount.toLocaleString()}</span>
              </div>
              <div>
                <span className="text-slate-300">Unique Rows:</span>
